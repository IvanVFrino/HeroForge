...
const Bestiary: React.FC = () => {
  ...
  const handleDeleteNpc = (npcId: string, npcName: string) => {
    if (window.confirm(`¿Estás seguro de que quieres eliminar a ${npcName}? Esta acción no se puede deshacer.`)) {
  ...
  const handleNpcAbilityCheckRoll = (npc: NPCData, abilityName: AbilityScoreName) => {
    const modifier = calculateModifier(npc.abilityScores[abilityName] || 10); // Fallback to 10 if score undefined
  ...
  const handleNpcSavingThrowRoll = (npc: NPCData, abilityName: AbilityScoreName) => {
    let modifier = calculateModifier(npc.abilityScores[abilityName] || 10); // Fallback for base modifier
    const specificSaveBonus = npc.savingThrows?.[abilityName];
    if (typeof specificSaveBonus === 'number') {
        modifier = specificSaveBonus; 
    }
    const result = performRoll(1, 20, modifier, attackRollMode);
            ...
            return (
                ...
                {isExpanded && (
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-center my-3 py-2 border-y border-slate-200 dark:border-slate-700">
                        {Object.entries(npc.abilityScores).map(([key, scoreValue]) => {
                            const ability = key as AbilityScoreName;
                            const score = typeof scoreValue === 'number' ? scoreValue : 10; // Fallback if score is not a number
                    ...
                    {npc.skills && Object.keys(npc.skills).length > 0 && (
                        <p><strong>Habilidades:</strong> {Object.entries(npc.skills).map(([key, val]) => `${getSkillDisplayName(key as SkillName)} ${Number(val) >= 0 ? '+' : ''}${val}`).join(', ')}</p>
                    )}
