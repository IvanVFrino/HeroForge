...
import { CharacterSheet, AbilityScoreName, SkillName, AbilityScores, Alignment, Trait, EquippedItem } from '../types';
import { SKILL_DEFINITIONS } from '../constants/skills'; 
import { getPcWeaponAttackBonus, getPcWeaponDamageDetails } from '../utils/characterCalculations';
import { DAMAGE_TYPE_ES_MAP } from '../../constants/displayMaps'; 
...
interface CharacterSummaryProps {
  ...
  onWeaponAttackRoll?: (weapon: EquippedItem) => void;
  onWeaponDamageRoll?: (weapon: EquippedItem, useVersatile: boolean) => void;
}
...
const CharacterSummary: React.FC<CharacterSummaryProps> = ({
  ...
  const {
    ...
    abilityScoreModifiers = {} as AbilityScores, 
    savingThrows = {}, skills = {},
    proficiencies = [], featuresAndTraits = [], equipment = []
  } = characterToDisplay;
  ...
  const equippedWeapons = equipment.filter(item => item.equipped && item.category === 'Weapon' && item.weaponDetails);
  ...
  return (
    <div className="p-1 bg-slate-200 dark:bg-slate-800 rounded-lg shadow-md text-slate-800 dark:text-slate-100">
      ...
      <SectionTitle>Ataques Equipados</SectionTitle>
      {equippedWeapons.length > 0 ? (
        <ul className="list-none space-y-1 text-xs mb-2">
          {equippedWeapons.map(weapon => {
            const attackBonus = onWeaponAttackRoll ? getPcWeaponAttackBonus(weapon, characterToDisplay) : 0;
            const damageDetails = onWeaponDamageRoll ? getPcWeaponDamageDetails(weapon, characterToDisplay, false) : null;
            const versatileDamageDetails = (onWeaponDamageRoll && weapon.weaponDetails?.versatileDamage) ? getPcWeaponDamageDetails(weapon, characterToDisplay, true) : null;

            return (
              <li key={weapon.instanceId} className="p-1.5 bg-slate-300/70 dark:bg-slate-700/70 rounded">
                <p className="font-medium text-slate-800 dark:text-slate-100">{weapon.name}</p>
                <div className="flex flex-wrap gap-1 mt-0.5">
                  {onWeaponAttackRoll && (
                    <button
                      onClick={() => onWeaponAttackRoll(weapon)}
                      className="flex items-center px-1.5 py-0.5 text-2xs bg-sky-600 hover:bg-sky-500 text-white rounded shadow"
                      aria-label={`Tirar ataque para ${weapon.name}`}
                    >
                      <GenericDieIcon className="h-3 w-3 mr-1"/> Ataque [{attackBonus >=0 ? '+' : ''}{attackBonus}]
                    </button>
                  )}
                  {onWeaponDamageRoll && damageDetails && (
                    <button
                      onClick={() => onWeaponDamageRoll(weapon, false)}
                      className="flex items-center px-1.5 py-0.5 text-2xs bg-red-600 hover:bg-red-500 text-white rounded shadow"
                      aria-label={`Tirar daño para ${weapon.name}`}
                    >
                       <GenericDieIcon className="h-3 w-3 mr-1"/> Daño [{damageDetails.dice}{damageDetails.modifier !== 0 ? (damageDetails.modifier > 0 ? `+${damageDetails.modifier}` : `${damageDetails.modifier}`) : ''} {DAMAGE_TYPE_ES_MAP[damageDetails.type] || damageDetails.type}]
                    </button>
                  )}
                  {onWeaponDamageRoll && versatileDamageDetails && (
                     <button
                      onClick={() => onWeaponDamageRoll(weapon, true)}
                      className="flex items-center px-1.5 py-0.5 text-2xs bg-orange-500 hover:bg-orange-400 text-white rounded shadow"
                      aria-label={`Tirar daño versátil para ${weapon.name}`}
                    >
                       <GenericDieIcon className="h-3 w-3 mr-1"/> Daño (V) [{versatileDamageDetails.dice}{versatileDamageDetails.modifier !== 0 ? (versatileDamageDetails.modifier > 0 ? `+${versatileDamageDetails.modifier}` : `${versatileDamageDetails.modifier}`) : ''} {DAMAGE_TYPE_ES_MAP[versatileDamageDetails.type] || versatileDamageDetails.type}]
                    </button>
                  )}
                </div>
              </li>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 italic">Ningún arma equipada o acciones de ataque no disponibles en este contexto.</p>
      )}


      <SectionTitle>Tiradas de Salvación</SectionTitle>
      <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 text-xs mb-2">
        {Object.entries(savingThrows).map(([key, stObj]) => {
          const abilityKey = key as AbilityScoreName;
          const st = stObj as { value: number, proficient: boolean } | undefined; 
          return st ? <DetailItem
            key={key}
            label={`${st.proficient ? '◉' : '○'} ${getAbilityDisplayName(abilityKey)}.`}
            value={`${(st.value ?? 0) >= 0 ? '+' : ''}${st.value ?? 'N/A'}`}
      ...
      <div className="grid grid-cols-1 gap-y-0.5 text-xs mb-2">
        {Object.entries(skills).map(([key, skillObj]) => {
          const skillKey = key as SkillName;
          const skill = skillObj as { value: number, proficient: boolean, ability: AbilityScoreName } | undefined; 
          return skill ? <DetailItem
            key={key}
            label={`${skill.proficient ? '◉' : '○'} ${getSkillDisplayName(skillKey)}`}
            value={`${(skill.value ?? 0) >= 0 ? '+' : ''}${skill.value ?? 'N/A'} (${getAbilityDisplayName(skill.ability as AbilityScoreName)})`}
