...
import { 
    NPCData, Size, Alignment, AbilityScoreName, SkillName, Trait, AbilityScores, ParsedNpcAttackAction, DamageType
} from '../../types';
...
import NpcTraitManager from './form_parts/NpcTraitManager';
import { ABILITY_SCORE_NAMES_ORDERED, DAMAGE_TYPES_CONST, SIZES_LIST, ALIGNMENTS_LIST } from '../../types';
...
const CreateCustomNpcForm: React.FC = () => {
    ...
    const [currentSpecialAbilityInput, setCurrentSpecialAbilityInput] = useState<CurrentTraitInput>(initialTraitInputState);
    const [currentActionInput, setCurrentActionInput] = useState<CurrentTraitInput>(initialTraitInputState);
    const [currentReactionInput, setCurrentReactionInput] = useState<CurrentTraitInput>(initialTraitInputState);
    const [currentLegendaryActionInput, setCurrentLegendaryActionInput] = useState<CurrentTraitInput>(initialTraitInputState);
    ...
    const addTraitToListWithInput = (
        targetListSetter: React.Dispatch<React.SetStateAction<Trait[]>>,
        currentInput: CurrentTraitInput,
        setCurrentInput: React.Dispatch<React.SetStateAction<CurrentTraitInput>>
    ) => {
        const { name, description, actionType, 
                attackBonus, damageDice, damageType, versatileDamageDice,
                saveDC, saveAbility, saveDamageDice, saveDamageType } = currentInput;

        if (!name.trim() || !description.trim()) {
            alert("El nombre y la descripción del rasgo son obligatorios."); return;
        }
        let parsedAttack: ParsedNpcAttackAction = {};
        if (actionType === 'attack') {
            if (attackBonus === undefined || !damageDice?.trim() || !damageType) { alert("Para acciones de ataque, se requieren Bonificación de Ataque, Dados de Daño y Tipo de Daño."); return; }
            parsedAttack.attack = { bonus: attackBonus };
            parsedAttack.hit = { diceString: damageDice.trim(), damageType: damageType, fullText: '' };
            if (versatileDamageDice?.trim()) parsedAttack.versatile = { diceString: versatileDamageDice.trim(), damageType: damageType, fullText: '' };
            ...
            if (saveDC === undefined || !saveAbility || !saveDamageDice?.trim() || !saveDamageType) { alert("Para efectos de tirada de salvación, se requieren DC, Característica de Salvación, Dados de Daño (en fallo) y Tipo de Daño."); return; }
            parsedAttack.savingThrow = { dc: saveDC, ability: saveAbility };
            parsedAttack.hit = { diceString: saveDamageDice.trim(), damageType: saveDamageType, fullText: '' };
        } else {
            const descParsed = parseNpcAttackAction(description.trim());
            if (descParsed) parsedAttack = descParsed;
        }
        const finalParsedAttack = (parsedAttack.attack || parsedAttack.hit || parsedAttack.savingThrow) ? parsedAttack : undefined;
        targetListSetter(prevList => [...prevList, { name: name.trim(), description: description.trim(), parsedAttack: finalParsedAttack }]);
        setCurrentInput(initialTraitInputState);
    };

    const handleRemoveTrait = (targetListSetter: React.Dispatch<React.SetStateAction<Trait[]>>, traitName: string) => {
    ...
    const resetFormFields = (includeAiPrompt = true) => {
        ...
        setCurrentSpecialAbilityInput(initialTraitInputState);
        setCurrentActionInput(initialTraitInputState);
        setCurrentReactionInput(initialTraitInputState);
        setCurrentLegendaryActionInput(initialTraitInputState);
    ...
    const handleGenerateWithAi = async () => {
        ...
        const systemPrompt = `
          ...
          {
            "name": "Nombre del PNJ (string)", "size": "Uno de: ${SIZES_LIST.join(' | ')}", "type": "Tipo (string, ej: 'Humanoide (humano)', 'Bestia', 'No muerto')",
            "alignment": "Uno de: ${ALIGNMENTS_LIST.join(' | ')}", "armorClass": "CA (number)", "acType": "Tipo de CA (string, ej: 'Armadura natural', '(Escudo)')",
            "hitPoints": "Puntos de Golpe (number)", "hitDice": "Dados de Golpe (string, ej: '4d8 + 4')", "speed": "Velocidad (string, ej: '30 pies, volar 60 pies')",
        ...
        try {
          ...
          setArmorClass(aiData.armorClass || 10); setAcType(aiData.acType || ''); setHitPoints(aiData.hitPoints || 10); setHitDice(aiData.hitDice || '');
          setSpeed(aiData.speed || '30 pies'); setAbilityScores({ ...initialAbilityScores, ...(aiData.abilityScores || {}) });
          
          const parsedSTs: Partial<Record<AbilityScoreName, number>> = {};
          if (Array.isArray(aiData.savingThrows)) aiData.savingThrows.forEach(st => { if (st.ability && ABILITY_SCORE_NAMES_ORDERED.includes(st.ability) && typeof st.bonus === 'number') parsedSTs[st.ability] = st.bonus; });
          setSavingThrows(parsedSTs);

          const parsedSkills: Partial<Record<SkillName, number>> = {};
          if (Array.isArray(aiData.skills)) aiData.skills.forEach(sk => { if (sk.skill && ALL_SKILL_NAMES_OBJECT.map(s=>s.name).includes(sk.skill) && typeof sk.bonus === 'number') parsedSkills[sk.skill] = sk.bonus; });
          setSkills(parsedSkills);

          setDamageVulnerabilities(aiData.damageVulnerabilities || []); setDamageResistances(aiData.damageResistances || []); setDamageImmunities(aiData.damageImmunities || []); setConditionImmunities(aiData.conditionImmunities || []);
          setSenses(aiData.senses || ''); setLanguages(aiData.languages || ''); setChallengeRating(aiData.challengeRating || '1/2'); setXp(aiData.xp);
          
          const processTraits = (aiTraits?: Array<Partial<Trait> & { parsedAttack?: ParsedNpcAttackAction }>): Trait[] => (aiTraits || []).map(t => ({ name: t.name || 'Rasgo sin nombre', description: t.description || '', parsedAttack: (t.parsedAttack && (t.parsedAttack.attack || t.parsedAttack.hit || t.parsedAttack.savingThrow)) ? t.parsedAttack : (parseNpcAttackAction(t.description || '') || undefined) }));
          setSpecialAbilities(processTraits(aiData.specialAbilities)); setActions(processTraits(aiData.actions)); setReactions(processTraits(aiData.reactions)); setLegendaryActions(processTraits(aiData.legendaryActions));
          setNpcDescription(aiData.description || ''); setSource(aiData.source || 'Personalizado (IA)');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!name.trim() || !type.trim() || !hitDice.trim() || !challengeRating.trim()) {
            alert('Nombre, Tipo, Dados de Golpe y Valor de Desafío son obligatorios.'); return;
        }
        const npcData: Omit<NPCData, 'id' | 'isCustom'> = {
            ...
            savingThrows: Object.keys(savingThrows).length > 0 ? savingThrows : undefined, skills: Object.keys(skills).length > 0 ? skills : undefined,
            damageVulnerabilities: damageVulnerabilities.length > 0 ? damageVulnerabilities : undefined, damageResistances: damageResistances.length > 0 ? damageResistances : undefined,
            damageImmunities: damageImmunities.length > 0 ? damageImmunities : undefined, conditionImmunities: conditionImmunities.length > 0 ? conditionImmunities : undefined,
            senses: senses.trim() || undefined, languages: languages.trim() || undefined, challengeRating: challengeRating.trim(), xp,
            specialAbilities, actions, reactions, legendaryActions, description: npcDescription.trim() || undefined, source: source.trim() || 'Personalizado',
        };
        dispatch({ type: 'ADD_CUSTOM_NPC', payload: npcData });
        alert(`¡PNJ "${name.trim()}" creado con éxito!`); resetFormFields(true);
    };
    
    const renderStringListManager = (
        title: string, list: string[], setList: React.Dispatch<React.SetStateAction<string[]>>,
        currentItem: string, setCurrentItem: React.Dispatch<React.SetStateAction<string>>, placeholder: string
    ) => (
         <div>
    ...
    return (
        <div className="container mx-auto p-4 md:p-6 max-w-4xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl">
                ...
                <NpcTraitManager title="Habilidades Especiales" list={specialAbilities} currentTraitInput={currentSpecialAbilityInput} setCurrentTraitInput={setCurrentSpecialAbilityInput} onAddTrait={() => addTraitToListWithInput(setSpecialAbilities, currentSpecialAbilityInput, setCurrentSpecialAbilityInput)} onRemoveTrait={(name) => handleRemoveTrait(setSpecialAbilities, name)} />
                <NpcTraitManager title="Acciones" list={actions} currentTraitInput={currentActionInput} setCurrentTraitInput={setCurrentActionInput} onAddTrait={() => addTraitToListWithInput(setActions, currentActionInput, setCurrentActionInput)} onRemoveTrait={(name) => handleRemoveTrait(setActions, name)} />
                <NpcTraitManager title="Reacciones" list={reactions} currentTraitInput={currentReactionInput} setCurrentTraitInput={setCurrentReactionInput} onAddTrait={() => addTraitToListWithInput(setReactions, currentReactionInput, setCurrentReactionInput)} onRemoveTrait={(name) => handleRemoveTrait(setReactions, name)} />
                <NpcTraitManager title="Acciones Legendarias" list={legendaryActions} currentTraitInput={currentLegendaryActionInput} setCurrentTraitInput={setCurrentLegendaryActionInput} onAddTrait={() => addTraitToListWithInput(setLegendaryActions, currentLegendaryActionInput, setCurrentLegendaryActionInput)} onRemoveTrait={(name) => handleRemoveTrait(setLegendaryActions, name)} />
