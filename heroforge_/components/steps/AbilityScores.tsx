...
const AbilityScoresStep: React.FC = () => {
  ...
  const determineInitialMethod = (): 'standardArray' | 'pointBuy' | 'roll' => {
    if (character.abilityScores && character._savedCoreDataHelper?.baseAbilityScores) {
        const sortedSavedBase = Object.values(character._savedCoreDataHelper.baseAbilityScores).sort((a,b) => b-a).join(',');
        const sortedStandardArray = [...STANDARD_ARRAY].sort((a,b) => b-a).join(',');
  ...
  useEffect(() => {
    ...
    if (character._savedCoreDataHelper?.baseAbilityScores) {
        ABILITY_SCORE_NAMES_ORDERED.forEach(absName => {
            const scoreVal = character._savedCoreDataHelper!.baseAbilityScores[absName];
        ...
        {ABILITY_SCORE_NAMES_ORDERED.map((ability) => (
          <div key={ability} className="flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-700 rounded-lg">
            ...
            <div className="flex flex-wrap gap-1 justify-end">
              {[...availableArrayScores, assignedArrayScores[ability]].filter(s => s !== undefined).sort((a,b)=>b-a).map(scoreValue => (
                <button
      ...
      <div className="mb-4">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            ABILITY_SCORE_NAMES_ORDERED.forEach(name => resetScores[name as AbilityScoreName] = 10);
